...
func (db *DB) NewsDetail(id int) ([]Post, error) {
	if id == 0 {
		return nil, fmt.Errorf("error id: %n", id)
	}

	query := fmt.Sprintf("SELECT id, title, content, pub_time, link FROM news WHERE id = %d", id)
